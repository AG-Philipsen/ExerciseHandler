...
\newcommand{\Heading}[1][true]{
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \shade[upper left=consolegray!70!black,lower right=consolegray] (NW1) -- (NW2) -- (NW3) -- cycle;
        \begin{scope}[scale=0.7]
            \coordinate (start) at ($(NW1)!0.75!(NWM)+(-8mm,0)$);
            \draw[thick, fill=white] (start) circle(1cm);
            \draw[thick, fill=white] ($(start)+(0,12mm)$) circle(.6);
            \draw ($(start)+(0,12.5mm)$) ++(-30:0.35) arc(-30:-150:0.35);
            \fill[black] ($(start)+(0,13mm)$) +(.1,0) circle(0.04) +(-0.1,0) circle(0.04);
            \fill[black] (start) ++(0,-1mm) circle(.1) +(0,4mm) circle(.1) +(0,-4mm) circle(.1);
            \draw[very thick] ($(start)+(20:8mm)$) -- ++(45:5mm);
            \draw[very thick] ($(start)+(160:8mm)$) -- ++(135:5mm);
        \end{scope}
        \node[anchor=south west, font=\scshape] at ($(start)+(4mm,4mm)$) {
            \begin{tabular}{c}
                \semester
            \end{tabular}
        };
